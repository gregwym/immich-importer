...
SOURCES = [('__init__',
 ...
 ('api',
  ...
  '            page = 1\n'
  '            while page:\n'
  '                data = self.request("POST", "/search/metadata", json={"originalFileName": '
  'candidate, "withExif": True, "size": 100, "page": page})\n'
  ...
  'isinstance(a.get("originalFileName"), str)\n'
  '                             and a["originalFileName"].casefold() == name.casefold() and '
  'a.get("ownerId") == self.owner_id\n'
  '                             and a.get("id") not in {f.get("id") for f in found})\n'
  '                page = int(assets["nextPage"]) if assets.get("nextPage") and page < 50 else '
  'None\n'
  '            if found:\n'
  '                break\n'
  '        return found\n'
 ...
 ('cli',
  ...
  '    delivery = "Immich extracts it from the file" if source.startswith("metadata:zoned") else '
  '"delivered in XMP DateTimeOriginal"\n'
  ...
  'ensure_ascii=False)\n'
  '                                  + ("  times " + json.dumps(item.get("embeddedTimeMetadata") '
 ...
 ('repair_time',
  ...
  '                    hashcache.record(index, item, now)\n'
  '                    try:\n'
  '                        hashcache.save(config.manifest_root, index, now)  # survive an '
  'interrupted run\n'
  '                    except (OSError, ImportFailure):\n'
